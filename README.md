  ...
  template:
    ...
    spec:
      ...
      - name: mc
        ...
        - /bin/sh
        - -c
        - "mc --config-dir=/config config host add velero http://minio:9000 minio minio123 && mc --config-dir=/config mb -p velero/velero && mc --config-dir=/config mb -p longhorn/velero"
        volumeMounts:
        - name: config
          mountPath: "/config"
EOF
